...
You can plot data freated with faux using `plot_design()`, too. It will return a ggplot with a violin-boxplot.

### One-factor data

```{r}
data <- sim_design(2, mu = 1:2, plot = FALSE)
plot_design(data)
```
...
```{r}
data <- sim_design(c(2,2,2), mu = 1:8, plot = FALSE)
abc <- plot_design(data, "A", "B", "C")
acb <- plot_design(data, "A", "C", "B")
bac <- plot_design(data, "B", "A", "C")
bca <- plot_design(data, "B", "C", "A")
cab <- plot_design(data, "C", "A", "B")
cba <- plot_design(data, "C", "B", "A")
...
```{r}
sim_design(c(2,2,2,5), mu = 1:40, plot = FALSE) %>% plot_design("D", "B", "C", "A")
```
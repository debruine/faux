...
get_params(df, between = "pet") %>% knitr::kable()
```

```{r, echo = FALSE, fig.width = 8, fig.height = 4}
gather(df, time, y, day:night) %>%
  ggplot(aes(pet, y, fill = pet)) +
...
df %>%
  ...
  ggplot(aes(version, y, fill = condition, grp = condition)) +
  geom_violin(show.legend = FALSE) +

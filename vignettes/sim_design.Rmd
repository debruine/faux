...
mu <- list(
  ...
  dog = c(day = 15, night = 25)
)
df <- sim_design(within, between, n = 100, mu = mu, sd = 5, r = 0.5)
```
...
You can specify the cells as rowname or column names and `check_design()` will fix them. Since `n` **has** to be the same for each within-subject factor, you can specify n as a single column with any name.

```{r}
n <- data.frame(n = c(10, 20, 30, 40),
                row.names = c("B1_C1", "B2_C1", "B1_C2", "B2_C2"))
design <- check_design(2, c(2,2), n = n, plot = FALSE)
str(design$n)
```
...
```{r}
mu <- list(
  cat_B = c(night = 60, day = 50),
  cat_A = c(day = 10, night = 20),
  dog_A = c(day = 30, night = 40),
...
mu <- data.frame(
  day = c(10, 30, 50, 70),
  night = c(20, 40, 60, 80),
  row.names = c("cat_A", "dog_A", "cat_B", "dog_B")
)
design <- check_design(within, between, mu = mu, plot = FALSE)
str(design$mu)
```

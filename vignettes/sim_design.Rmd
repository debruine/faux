...
In wide format, faux uses an underscore to separate level names. Therefore, any underscores in factor level names will be replaced with '.'.
...
You can change this default with `faux_options()`. If the separator is set to '.', all '.' in names will be changed to '_'.
...
If you output your data in long format, faux won't mangle the names, regardless of the separator value.

```{r}
faux_options(sep = "_")

sim_design(within, n = 2, long = TRUE, plot = FALSE)
```

...
Have you ever wanted to create unique versions of datasets for each student in a class? This vignette walks through the proess of setting up a simple study with a subject table and a questionnaire table. 
...
Some of the functions below currently require the development version of faux (`add_random()`, `add_between()`, `add_ranef()` and `add_contrast()`). I am working on submitting the new version to CRAN, so any feedback would be welcome.

```{r setup, message=FALSE}
# devtools::install_github("debruine/faux", ref = "dev")
library(faux)
library(dplyr)
library(tidyr)
library(purrr)
...
```{r}
research_question <- readLines("template_instructions.txt") %>%
  paste(collapse = "\n") %>%
  glue::glue()
```
...
random_project <- function(student_id) {
  ...
  # sample research question variables
  factors <- sample(c("language", "condition", "version"), 2)
  iv <- factors[[1]]
  subset <- factors[[2]]
  level <- subjects[[subset]] %>% unique() %>% sample(1)

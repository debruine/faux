...
|:----------------------|:--------------------|:----------|
| Treatment | Dummy | `contr.treatment` |
| Deviation | Simple, Orthogonal (Davis), Contrast (Barlaz) | `contr.treatment - 1/n` |
| Successive differences | Contrast | `MASS::contr.sdif` |
| Sum | Effects (Barlaz)| `contr.sum` |
| Helmert | Reverse helmert (Barlaz), Trend (Davis) | `contr.helmert` |
| Polynomial | Trend (Davis) | `contr.poly` |
...
A slightly different form of contrast coding is successive differences. It compares each level to the previous one, rather than to a baseline level.
...
df$pet <- sdif_code(df$pet, levels = c("cat", "dog", "ferret"))
```

...
You can build up a mixed effects model by adding random factors in a stepwise fashion using `add_random()`. The example below simulates 3 schools with 2 classes nested in each school, then a different number of pupils nested in each class, and finally crossed with 10 questions.

```{r add-random}
data <- add_random(NULL, school = 3) %>%
  add_random(class = 2, nested_in = "school") %>%
  add_random(pupil = c(20, 24, 23, 21, 25, 24), nested_in = "class") %>%
  add_random(question = 10)
```

A cross-classified design with 100 subjects and 20 items.

```{r}
data <- add_random(NULL, subj = 100, item = 20)
```
...
```{r}
data <- add_random(NULL, subj = 4, item = 2) %>%
  add_between("subj", cond = c("control", "test"), gen = c("X", "Z")) %>%
  add_between("item", version = c("A", "B")) %>%
  add_between(c("subj", "item"), x = 1:4, shuffle = TRUE) %>%
  add_between(c("subj", "item"), y = 1:4, shuffle = FALSE)
```
...
You can use the `filter()` function from dplyr to create counterbalanced designs. For example, if your items are grouped into A and B counterbalanced groups, add a between-subjects and a between-items factor with the same levels and filter only rows where the subject value matches the item value. In the example below, subjects 1-3 only see the odd-numbered items, while subjects 4-6 only see the even-numbered items.

```{r}
data <- add_random(NULL, subj = 6, item = 6) %>%
...
The code below sets up a simple cross-classified design where 2 subjects are crossed with 2 items. You can use the function `add_ranef()` to add random effects for subject, item, and their interaction, as well as an error term (`sigma`). If you add more than one random effect for the same group (e.g., a random intercept and a random slope), you can specify their correlation with `.cors` (you can specify correlations in the same way as for `rnorm_multi()`).
...
sim <- function(subj_n = 10, item_n = 10,
                ...
                u1i_sd = 1, r01i = 0,   # random slope and correlation

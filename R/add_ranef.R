...
#' @param .grp the grouping column (groups by row if NULL)
...
add_ranef <- function(data, .grp = NULL, ..., .cors = 0, empirical = FALSE) {
  ...
  dplyr::left_join(data, ranefs, by = .grp)
}
...
#' data5 <- sim_design(within = 2, n = 3, long = TRUE) %>%
#'   add_random(Q = 2)
#'   
...
add_between <- function(data, .grp = NULL, ..., shuffle = FALSE, prob = NULL) {
  if (is.null(.grp)) {
    .grp <- names(data)
    grps <- data
  } else {
  ...
  dplyr::left_join(data, grps, by = .grp)
}
...
#' @examples
#' 
#' data <- add_random(NULL, subj = 2, item =  2) %>%
#'   add_within("subj", time = c("pre", "post"))
add_within <- function(data, .grp = NULL, ...) {
  if (is.null(.grp)) {
    .grp <- names(data)
    grps <- data
  } else {
  ...
  dplyr::left_join(data, newdat, by = .grp)
}

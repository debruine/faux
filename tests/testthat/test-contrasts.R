test_that("contrast_code", {
  x <- factor(1:2, labels = LETTERS[1:2])
  x1 <- contrast_code(x, base = 1)
  x2 <- contrast_code(x, base = 2)
  mat1 <- matrix(c(-0.5, 0.5), dimnames = list(LETTERS[1:2], "_B.vs.A"))
  mat2 <- matrix(c(0.5, -0.5), dimnames = list(LETTERS[1:2], "_A.vs.B"))
  expect_equal(contrasts(x1), mat1)
  expect_equal(contrasts(x2), mat2)
  
  x <- factor(1:3, labels = LETTERS[1:3])
  x1 <- contrast_code(x, base = 1)
  x2 <- contrast_code(x, base = 2)
  x3 <- contrast_code(x, base = 3)
  mat1 <- matrix(c(-1/3,  2/3, -1/3, -1/3, -1/3, 2/3), nrow = 3,
                 dimnames = list(LETTERS[1:3], c("_B.vs.A", "_C.vs.A")))
  mat2 <- matrix(c(2/3, -1/3, -1/3, -1/3, -1/3, 2/3), nrow = 3,
                 dimnames = list(LETTERS[1:3], c("_A.vs.B", "_C.vs.B")))
  mat3 <- matrix(c(2/3, -1/3, -1/3, -1/3, 2/3, -1/3), nrow = 3,
                 dimnames = list(LETTERS[1:3], c("_A.vs.C", "_B.vs.C")))
  expect_equal(contrasts(x1), mat1)
  expect_equal(contrasts(x2), mat2)
  expect_equal(contrasts(x3), mat3)
})

# contrast_code, base by name ----
test_that("contrast_code, base by name", {
  x <- factor(1:2, labels = LETTERS[1:2])
  xA <- contrast_code(x, base = "A")
  xB <- contrast_code(x, base = "B")
  mat1 <- matrix(c(-0.5, 0.5), dimnames = list(LETTERS[1:2], "_B.vs.A"))
  mat2 <- matrix(c(0.5, -0.5), dimnames = list(LETTERS[1:2], "_A.vs.B"))
  expect_equal(contrasts(xA), mat1)
  expect_equal(contrasts(xB), mat2)
})

# contrast_code, specify levels ----
test_that("contrast_code, specify levels", {
  x <- 1:2
  lvls <- c("A", "B")
  x1 <- contrast_code(x, levels = lvls, base = 1)
  x2 <- contrast_code(x, levels = lvls, base = 2)
  mat1 <- matrix(c(-0.5, 0.5), dimnames = list(LETTERS[1:2], "_B.vs.A"))
  mat2 <- matrix(c(0.5, -0.5), dimnames = list(LETTERS[1:2], "_A.vs.B"))
  expect_equal(contrasts(x1), mat1)
  expect_equal(contrasts(x2), mat2)
  
  
  x <- c("A", "B")
  lvls <- c("B", "A")
  x1 <- contrast_code(x, levels = lvls, base = 1)
  x2 <- contrast_code(x, levels = lvls, base = 2)
  mat1 <- matrix(c(-0.5, 0.5), dimnames = list(LETTERS[2:1], "_A.vs.B"))
  mat2 <- matrix(c(0.5, -0.5), dimnames = list(LETTERS[2:1], "_B.vs.A"))
  expect_equal(contrasts(x1), mat1)
  expect_equal(contrasts(x2), mat2)
})

# sum_code ----
test_that("sum_code", {
  x <- factor(1:2, labels = LETTERS[1:2])
  x0 <- sum_code(x)
  x1 <- sum_code(x, omit = 1)
  x2 <- sum_code(x, omit = 2)
  mat1 <- matrix(c(-1, 1), dimnames = list(LETTERS[1:2], "_B.vs.intercept"))
  mat2 <- matrix(c(1, -1), dimnames = list(LETTERS[1:2], "_A.vs.intercept"))
  expect_equal(contrasts(x0), mat2)
  expect_equal(contrasts(x1), mat1)
  expect_equal(contrasts(x2), mat2)
  
  x <- factor(1:3, labels = LETTERS[1:3])
  x1 <- sum_code(x, omit = 1)
  x2 <- sum_code(x, omit = 2)
  x3 <- sum_code(x)
  
  expect_equal(contrasts(x1) %>% as.vector(), c(0, -1, 1, 1, -1, 0))
  expect_equal(contrasts(x2) %>% as.vector(), c(1, -1, 0, 0, -1, 1))
  expect_equal(contrasts(x3) %>% as.vector(), c(1, 0, -1, 0, 1, -1))
  expect_equal(
    contrasts(x1) %>% dimnames() %>% `[[`(2), 
    c("_B.vs.intercept", "_C.vs.intercept")
  )
  expect_equal(
    contrasts(x2) %>% dimnames() %>% `[[`(2), 
    c("_A.vs.intercept", "_C.vs.intercept")
  )
  expect_equal(
    contrasts(x3) %>% dimnames() %>% `[[`(2), 
    c("_A.vs.intercept", "_B.vs.intercept")
  )
})


# treatment_code ----
test_that("treatment_code", {
  x <- factor(1:3, labels = LETTERS[1:3])
  x1 <- treatment_code(x)
  x2 <- treatment_code(x, base = "B")
  x3 <- treatment_code(x, base = 3)
  mat1 <- matrix(c(0,1,0,0,0,1), 3, 
                 dimnames = list(LETTERS[1:3], c("_B.vs.A", "_C.vs.A")))
  mat2 <- matrix(c(1,0,0,0,0,1), 3, 
                 dimnames = list(LETTERS[1:3], c("_A.vs.B", "_C.vs.B")))
  mat3 <- matrix(c(1,0,0,0,1,0), 3, 
                 dimnames = list(LETTERS[1:3], c("_A.vs.C", "_B.vs.C")))
  expect_equal(contrasts(x1), mat1)
  expect_equal(contrasts(x2), mat2)
  expect_equal(contrasts(x3), mat3)
})


# helmert_code ----
test_that("helmert_code", {
  x <- factor(1:3, labels = LETTERS[1:3])
  x1 <- helmert_code(x)
  x2 <- helmert_code(x, levels = c("C", "B", "A"))
  mat1 <- matrix(c(-.5, .5, 0, -1/3, -1/3, 2/3), 3, 
                 dimnames = list(LETTERS[1:3], c("_B.vs.A", "_C.vs.A.B")))
  mat2 <- matrix(c(-.5, .5, 0, -1/3, -1/3, 2/3), 3, 
                 dimnames = list(LETTERS[3:1], c("_B.vs.C", "_A.vs.C.B")))
  expect_equal(contrasts(x1), mat1)
  expect_equal(contrasts(x2), mat2)
})
  